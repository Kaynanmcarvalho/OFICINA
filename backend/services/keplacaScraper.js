...
async function scrapeKeplaca(plate) {
    ...
    try {
        ...
        const data = await page.evaluate(() => {
            ...
            // Extrai Cor - regex melhorado para capturar cores com espaços e acentos
            // Tenta múltiplos padrões
            let corMatch = bodyText.match(/Cor:\s*([A-Za-zÀ-ÿ\s]+?)(?=\s*Combustível:|Chassi:|Tipo:|$)/i);
            if (!corMatch) {
                // Tenta padrão alternativo - cor seguida de quebra de linha
                corMatch = bodyText.match(/Cor:\s*([A-Za-zÀ-ÿ\s]+?)[\n\r]/i);
            }
            if (!corMatch) {
                // Tenta padrão mais simples - apenas a primeira palavra após "Cor:"
                corMatch = bodyText.match(/Cor:\s*([A-Za-zÀ-ÿ]+)/i);
            }
            if (corMatch) result.cor = corMatch[1].trim();

            // Extrai Combustível
            const combustivelMatch = bodyText.match(/Combustível:\s*([A-Za-zÀ-ÿ\/\s]+?)(?=\s*Chassi:|UF:|Tipo:|$)/i);
            ...
            // Extrai Município
            const municipioMatch = bodyText.match(/Município:\s*([A-ZÀ-ÿ\s]+?)(?=\s*Esta placa|Valores FIPE|$)/i);
            if (municipioMatch) result.municipio = municipioMatch[1].trim();

            // Limpa dados
            Object.keys(result).forEach(key => {
                if (typeof result[key] === 'string' && key !== '_debug') {
        ...
        console.log(`[KEPLACA] 🎨 Cor: ${data.cor || 'NÃO ENCONTRADA'}`);
