...
interface RecentItemInnovativeProps {
  item: RecordItem;
  isSelected?: boolean;
...
const RecentItemInnovative: React.FC<RecentItemInnovativeProps> = ({
  item,
  isSelected = false,
  onSelect,
  onClick,
  ...
  // Layout variations based on vehicle type
  ...
  const getThemeColors = () => {
    ...
    if (isDark) {
      ...
      return {
        cardBg: 'bg-white/95 dark:bg-gray-50/95',
        border: 'border-gray-200/50 dark:border-gray-300/50',
        text: 'text-gray-900',
    ...
    >
      <div 
        className={`
          ...
          ${isSelected ? `ring-2 ring-${themeColors.accent}-500/50` : ''}
          transition-all duration-300 ease-out
          hover:shadow-xl hover:border-${themeColors.accent}-500/30
        `}
      ...
      >
        {/* Selection indicator */}
        {isSelected && (
          <div className={`absolute top-2 right-2 w-3 h-3 bg-${themeColors.accent}-500 rounded-full`} />
        )}
            ...
            >
              <label className="cursor-pointer">
                <input
                  ...
                  className={`w-5 h-5 rounded border-2 border-${themeColors.accent}-500 text-${themeColors.accent}-500 focus:ring-${themeColors.accent}-500`}
                />
              </label>
            </motion.div>
          )}
        ...
        <div 
          className={`
            ...
            from-${themeColors.accent}-500 to-${themeColors.accent}-600 
...
export default RecentItemInnovative;